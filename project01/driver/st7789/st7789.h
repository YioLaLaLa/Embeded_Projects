...
/* SPI2 由 spi_init() 配置：SCK=PB13, MOSI=PB15（LCD 只写，MISO=PB14 不用）。
   控制引脚按板载 TFT 座 P2 实际接线：CS=PB12, DC=PC5, BLK=PB1；RST 不在 P2 上，杜邦线接 PB8（空闲脚）。
   DC 在 GPIOC，st7789.c 的 gpio_init 已分别初始化 GPIOB / GPIOC。 */
...
#define ST7789_RST_PIN    GPIO_Pin_8      /* 硬件复位，低有效（杜邦线接 PB8，空闲脚无冲突）*/
